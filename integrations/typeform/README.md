...
4. Copy [.env.example](.env.example) with the name `.env`, and add/update the environment variables there. (**Note: Keep `AUTH_JWT_SECRET` values same through every project for bettercollected**)
5. After configuring the environment variables, the application is ready to run
6. Run `uvicorn typeform.app:get_application --port 8002 --reload --env-file .env`
