...
.. code-block:: bash
    ...
    $ googleform --help
    Usage: googleform [OPTIONS] COMMAND [ARGS]...
    ...
    Commands:
      serve  googleform CLI serve command.
    $ googleform serve --help
    Usage: googleform serve [OPTIONS]
...
.. code-block:: bash
    ...
    $ make image
    $ podman run -dit --name googleform -p 8000:8000 googleform:$(cat TAG)
    f41e5fa7ffd512aea8f1aad1c12157bf1e66f961aeb707f51993e9ac343f7a4b
    $ podman ps
    CONTAINER ID  IMAGE                                 COMMAND               CREATED        STATUS            PORTS                   NAMES
    f41e5fa7ffd5  localhost/googleform:0.1.0  /usr/bin/fastapi ...  2 seconds ago  Up 3 seconds ago  0.0.0.0:8000->8000/tcp  googleform
    $ curl localhost:8000/api/ready
    {"status":"ok"}
...
You can implement your own web routes logic straight away in ``googleform.controllers`` submodule. For more information please see `FastAPI documentation <https://fastapi.tiangolo.com/tutorial/>`__.
...
.. code-block:: bash
    ...
    Usage:
      ...
      image            Build googleform image
      clean-image      Clean googleform image
      install          Install googleform with poetry
      metrics          Run googleform metrics checks
      unit-test        Run googleform unit tests
      integration-test  Run googleform integration tests
      docs             Build googleform documentation
...
* RedisClient ``googleform.app.utils.redis``
* AiohttpClient ``googleform.app.utils.aiohttp_client``
...
.. code-block:: python
    ...
    from googleform.app.utils import RedisClient
...
.. literalinclude:: ../googleform/app/exceptions/http.py
    :language: python
...
All application configuration is available in ``googleform.config`` submodule.
...
.. literalinclude:: ../googleform/config/gunicorn.py
    :language: python

Routes
~~~~~~

Endpoints are defined in ``googleform.app.router`` submodule. Just simply import your controller and include it to FastAPI router:

.. literalinclude:: ../googleform/app/router.py
    :language: python
