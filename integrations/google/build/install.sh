...
Now you should access CLI script: $ googleform --help
Alternatively you can access CLI script via poetry run: $ poetry run googleform --help
To deactivate virtualenv simply type: $ deactivate
To activate shell completion:
 - for bash: $ echo 'eval "$(_GOOGLEFORM_COMPLETE=source_bash googleform)' >> ~/.bashrc
 - for zsh: $ echo 'eval "$(_GOOGLEFORM_COMPLETE=source_zsh googleform)' >> ~/.zshrc
 - for fish: $ echo 'eval "$(_GOOGLEFORM_COMPLETE=source_fish googleform)' >> ~/.config/fish/completions/googleform.fish
EOF
