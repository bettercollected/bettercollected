...
minikube image load "googleform:${TAG}"

echo "[dev-env] creating googleform namespace"
kubectl create namespace googleform

ATTEMPTS=0
ROLLOUT_STATUS_CMD="kubectl get namespace googleform -n googleform"
...
kubectl create -f manifests/persistent-storage-no-pvc-deletion.yaml -n googleform

ATTEMPTS=0
ROLLOUT_STATUS_CMD="kubectl rollout status deployment.apps/rfs-redisfailover-persistent-keep -n googleform"
until $ROLLOUT_STATUS_CMD || [ $ATTEMPTS -eq 60 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 10
done
...
STATEFULSET_REPLICAS=$(kubectl get statefulset rfr-redisfailover-persistent-keep -o jsonpath='{.spec.replicas}' -n googleform)
ATTEMPTS=0
until [[ ${STATEFULSET_REPLICAS} -eq $(kubectl get statefulset rfr-redisfailover-persistent-keep -o jsonpath='{.status.readyReplicas}' -n googleform) ]] || [ $ATTEMPTS -eq 60 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 10
done

echo "[dev-env] installing googleform charts"
helm upgrade --install \
    googleform charts/googleform \
    --namespace googleform \
    --set ingress.host.name="googleform.${INGRESS_HOST}"

ATTEMPTS=0
ROLLOUT_STATUS_CMD="kubectl rollout status deployment/googleform -n googleform"
until $ROLLOUT_STATUS_CMD || [ $ATTEMPTS -eq 60 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 10
done
...
FastAPI available under: http://googleform.${INGRESS_HOST}/
