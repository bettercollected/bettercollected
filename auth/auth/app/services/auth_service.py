...
class AuthService:
    ...
    def get_logged_user(jwt_token: str) -> User:
        jwt_response = jwt.decode(
            jwt_token, key=settings.JWT_SECRET, algorithms=["HS256"]
        )
        user = User(**jwt_response)
        return user

    async def get_credential(self, *, email, provider) -> Credential:
        credential = await self.credential_repository.get_credential(
            email=email, provider=provider
        )
        return Credential(**credential)

    async def get_basic_auth_url(
        self, provider_name: str, client_referer_url: str
    ) -> str:
        provider = await self.provider_repository.get_provider(provider_name)
        auth_redirection_url = await self.http_client.get(
            provider.basic_auth_url, headers={"referer": client_referer_url}
        )
        return auth_redirection_url

    async def get_oauth_url(self, provider_name: str, oauth_state: OAuthState):
        ...
        oauth_redirection_url = await self.http_client.get(
            provider.oauth_url, params=oauth_state.dict()
        )
    ...
    async def handle_auth_callback(self, jwt_token: str):
        ...
        await self.http_client.get(
            oauth_state.backend_auth_redirect_uri, params={"jwt_token": token}
        )
