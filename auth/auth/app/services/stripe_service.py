...
class StripeService:
    ...
    async def webhooks(self, request: Request):
        ...
        try:
            event = stripe.Webhook.construct_event(payload=body, sig_header=stripe_signature, secret=webhook_secret)
            data = event['data']
        ...
        if event_type == 'checkout.session.completed':
            ...
            user: UserDocument = await self.user_repository.get_user_by_stripe_payment_id(stripe_payment_id)
            user.stripe_customer_id = stripe_customer_id
            user.plan = Plans.PRO
            await user.save()
        elif event_type == 'customer.subscription.deleted':
            stripe_customer_id = data.object.customer
            user: UserDocument = await self.user_repository.get_user_by_stripe_customer_id(stripe_customer_id)
