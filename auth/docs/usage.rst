...
.. code-block:: bash
    ...
    $ auth --help
    Usage: auth [OPTIONS] COMMAND [ARGS]...
    ...
    Commands:
      serve  auth CLI serve command.
    $ auth serve --help
    Usage: auth serve [OPTIONS]
...
.. code-block:: bash
    ...
    $ make image
    $ podman run -dit --name auth -p 8000:8000 auth:$(cat TAG)
    f41e5fa7ffd512aea8f1aad1c12157bf1e66f961aeb707f51993e9ac343f7a4b
    $ podman ps
    CONTAINER ID  IMAGE                                 COMMAND               CREATED        STATUS            PORTS                   NAMES
    f41e5fa7ffd5  localhost/auth:0.1.0  /usr/bin/fastapi ...  2 seconds ago  Up 3 seconds ago  0.0.0.0:8000->8000/tcp  auth
    $ curl localhost:8000/api/ready
    {"status":"ok"}
...
You can implement your own web routes logic straight away in ``auth.controllers`` submodule. For more information please see `FastAPI documentation <https://fastapi.tiangolo.com/tutorial/>`__.
...
.. code-block:: bash
    ...
    Usage:
      ...
      clean-image      Clean auth image
      install          Install auth with poetry
      metrics          Run auth metrics checks
      unit-test        Run auth unit tests
      integration-test  Run auth integration tests
      docs             Build auth documentation
...
* RedisClient ``auth.app.utils.redis``
* AiohttpClient ``auth.app.utils.aiohttp_client``
...
.. code-block:: python
    ...
    from auth.app.utils import RedisClient
...
.. literalinclude:: ../auth/app/exceptions/http.py
    :language: python
...
All application configuration is available in ``auth.config`` submodule.
...
.. literalinclude:: ../auth/config/gunicorn.py
    :language: python

Routes
~~~~~~

Endpoints are defined in ``auth.app.router`` submodule. Just simply import your controller and include it to FastAPI router:

.. literalinclude:: ../auth/app/router.py
    :language: python
