...
Now you should access CLI script: $ auth --help
Alternatively you can access CLI script via poetry run: $ poetry run auth --help
To deactivate virtualenv simply type: $ deactivate
To activate shell completion:
 - for bash: $ echo 'eval "$(_AUTH_COMPLETE=source_bash auth)' >> ~/.bashrc
 - for zsh: $ echo 'eval "$(_AUTH_COMPLETE=source_zsh auth)' >> ~/.zshrc
 - for fish: $ echo 'eval "$(_AUTH_COMPLETE=source_fish auth)' >> ~/.config/fish/completions/auth.fish
EOF