        ...
        <div className={'flex flex-col gap-20 px-20 py-10 justify-center'}>
            {Array.isArray(slideFields) && slideFields.length ? slideFields.map((field, index) => {
                return <div key={index} tabIndex={0}
                    className={cn(activeFieldComponent?.id === field.id && "ring-1 ring-blue-500", "p-1 w-fit")}
                    onClick={(event) => {
                    ...
                    <div className={'flex flex-col items-start'}>
                        <div className="flex gap-2 items-center">
                            {
                                activeSlide?.properties?.showQuestionNumbers && <span className="text-2xl">
                                    {index + 1}.
                                </span>
                            }

                            <input id={`input-${disabled ? `${slide.id}${field.id}` : field.id}`}
                                placeholder={getPlaceholderValueForTitle(field.type || FieldTypes.SHORT_TEXT)} type="text"
...
const InputField = ({ field, slide, disabled }: { field: FormField, slide: FormField, disabled: boolean }) => {
    ...
    return <>
        ...
        }} type={field.type} value={field.properties?.placeholder} placeholder={getPlaceholderValueForField(field.type || FieldTypes.SHORT_TEXT)}
...
const YesNoField = ({ field, slide, disabled }: { field: FormField, slide: FormField, disabled: boolean }) => {
    return <>
        <RadioGroup className={'flex flex-col gap-2 w-full'} value={field.value} onChange={() => {
        }}>
...
const DropDownField = ({ field, slide, disabled }: { field: FormField, slide: FormField, disabled: boolean }) => {
    ...
    return <>
        ...
        {field.type === FieldTypes.DROP_DOWN ?
            <div
                className='flex justify-between w-full py-2 text-2xl text-brand-500 items-center border-0 border-brand-500 border-b-[1px] mb-2'>
                <h1>Select an option</h1> <ArrowDown className='stroke-2' /></div> : <div>Choose as many you like</div>
        }
        <div className={'flex flex-col gap-2 w-full'}>
            {field && field.properties?.choices?.map((choice, index) => {
                return <input type='text' value={choice.value} key={index} placeholder={`Item ${index + 1}`}
        ...
        <Button onClick={() => addChoiceField(field.index, slide.index)} variant={'ghost'}
            className='text-lg font-semibold mt-2' icon={<PlusIcon className='h-4 w-4' />}>Add Option</Button>
    </>
}
