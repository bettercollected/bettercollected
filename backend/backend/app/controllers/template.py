...
class FormTemplateRouter(Routable):
    ...
    async def create_new_template(self, workspace_id: PydanticObjectId, template_body: str = Form(),
                                  logo: UploadFile = None,
                                  cover_image: UploadFile = None,
                                  user: User = Depends(get_logged_user)):
        template = json.loads(template_body)
        response = await self.form_template_service.create_new_template(workspace_id=workspace_id, user=user,
    ...
    @patch('/workspaces/{workspace_id}/template/{template_id}', response_model=StandardFormTemplate)
    async def update_template(self, workspace_id: PydanticObjectId, template_id: PydanticObjectId,
                              logo: UploadFile = None,
                              cover_image: UploadFile = None,
        ...
        template = json.loads(template_body)
        response = await self.form_template_service.update_template(workspace_id=workspace_id, template_id=template_id,
                                                                    user=user,
    ...
    async def delete_template(self, workspace_id: PydanticObjectId, template_id: PydanticObjectId,
                              user: User = Depends(get_logged_user)):
        response = await self.form_template_service.delete_template(workspace_id=workspace_id, template_id=template_id,
                                                                    user=user)
        return response
