...
class FormResponseService:
    ...
    async def get_workspace_submission(
        ...
        mapped_form_response = FormResponseService._map_typeform_form_and_response(
            form, response
        )
                ...
                if not question.type.get("type") == "GROUP":
                    ...
                    if question.type.get("questions") is None:
                        ...
                        for sub_question in question.type["questions"]:
                            answer_field = response.responses.get(
                                sub_question.get("questionId")
                            )

                            if sub_question.get("type").get("type") == "RADIO":
                                answer = (
                                    answer_field.answer[0].get("value")
                                ...
                                for a in answer_field.answer:
                                    answer.append(
                                        a.get("value") if type(a) is dict else a
                                    )
                            else:
