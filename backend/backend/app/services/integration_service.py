...
class IntegrationService:
    def __init__(self,
        ...
        self.integration__provider_factory = IntegrationProviderFactory(form_provider_service, crypto, http_client,
                                                                        integration_action_service)

    async def get_oauth_url(self, integration_type: FormIntegrationType, client_referer_url: str, user: User):
        return await self.integration__provider_factory.get_integration_provider(
            integration_type).get_basic_integration_oauth_url(client_referer_url=client_referer_url)

    async def handle_oauth_callback(self, integration_type: FormIntegrationType, state: str, code: str, user: User,
                                    form_id: str, action_id: str):
        return await self.integration__provider_factory.get_integration_provider(
            integration_type).handle_basic_integration_callback(state=state, code=code, form_id=form_id,
                                                                action_id=action_id)
