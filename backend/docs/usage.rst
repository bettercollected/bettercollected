...
.. code-block:: bash
    ...
    Commands:
      serve  bettercollected-backend-server CLI serve command.
    $ bettercollected-backend-server serve --help
    Usage: bettercollected-backend-server serve [OPTIONS]
...
.. code-block:: bash
    ...
    bettercollected-backend-server serve
...
.. code-block:: bash
    ...
    $ make image
    $ podman run -dit --name bettercollected-backend-server -p 8000:8000 bettercollected-backend-server:$(cat TAG)
    f41e5fa7ffd512aea8f1aad1c12157bf1e66f961aeb707f51993e9ac343f7a4b
    $ podman ps
    CONTAINER ID  IMAGE                                 COMMAND               CREATED        STATUS            PORTS                   NAMES
    f41e5fa7ffd5  localhost/bettercollected-backend-server:0.1.0  /usr/bin/fastapi ...  2 seconds ago  Up 3 seconds ago  0.0.0.0:8000->8000/tcp  bettercollected-backend-server
    $ curl localhost:8000/api/ready
    {"status":"ok"}
...
.. code-block:: bash
    ...
    Usage:
      ...
      clean-image      Clean bettercollected-backend-server image
      install          Install bettercollected-backend-server with poetry
      metrics          Run bettercollected-backend-server metrics checks
      unit-test        Run bettercollected-backend-server unit tests
      integration-test  Run bettercollected-backend-server integration tests
      docs             Build bettercollected-backend-server documentation
