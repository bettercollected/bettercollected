...
import FormBuilderTitleDescriptionInput
    from '@Components/FormBuilder/TitleAndDescription/FormBuilderTitleDescriptionInput';
...
export default function FormBuilder({workspace, _nextI18Next, isTemplate = false, templateId}: {
    ...
    const onFormSave = async (builderState: any, consentState: any, headerImages: any, imagesRemoved: any) => {
    ...
    const saveFormDebounced = useCallback(
        _.debounce((builderState, consentState, headerImages, imagesRemoved) => onFormSave(builderState, consentState, headerImages, imagesRemoved), 2000),
        []
    );
    ...
    useEffect(() => {
        if (!isTemplate) {
            ...
            saveFormDebounced(builderState, consentState, headerImages, imagesRemoved);
        }
            ...
            <div className="h-full w-full max-w-4xl mx-auto py-12">
                ...
                <div className="flex flex-col gap-2 px-12 md:px-[89px]">
                    {builderTitleAndDescriptionList.map((b: IBuilderTitleAndDescriptionObj) => (
                        <FormBuilderTitleDescriptionInput key={b.id} b={b} value={builderState[b.key]}
                                                          onChangeCallback={onTitleDescriptionChangeCallback}
                                                          onFocusCallback={onTitleDescriptionFocusCallback}/>
                    ))}
                </div>
                ...
                <AnimatePresence mode="wait" initial={false}>
                    {showSaving.status && (
                        ...
                        >
                            <div
                                className="fixed px-5 py-2 rounded  bg-black-200 text-black-800 bottom-5 right-5 flex justify-center w-[120px] gap-2 lg:bottom-10 lg:right-10">
                                {showSaving.text === 'Saving' ? <CircularProgress size={24}/> :
                                    <Check className="text-green-500" height={24} width={24}/>}
                                {showSaving.text}
                            </div>
                        </motion.div>
                    )}
