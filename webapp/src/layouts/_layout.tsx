...
export function Header(props: any) {
    ...
    return <nav className={`fixed top-0 !z-30 flex w-full items-center justify-between px-4 transition-all duration-300 ltr:right-0 rtl:left-0 sm:px-6 lg:px-8 xl:px-10 3xl:px-12 h-16 sm:h-[68px] ${navClassNames}`}>{props.children}</nav>;
}
...
export default function Layout({ children, className = '', showNavbar = false }: React.PropsWithChildren<LayoutProps>) {
    return (
        <div className="!min-h-full !min-w-full bg-brand-100 dark:bg-dark z-20">
            ...
            <main className={`relative mb-0 px-4 ${showNavbar ? 'pt-16 sm:pt-[68px]' : ''} sm:px-6 sm:pb-20 lg:px-8 xl:px-10 3xl:px-12 ${className}`}>{children}</main>
        </div>
    );
}
