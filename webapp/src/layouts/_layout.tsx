...
export default function Layout({ children, hideMenu = false, showHamburgerIcon = false, className = '', showNavbar = false, showAuthAccount }: React.PropsWithChildren<LayoutProps>) {
    return (
        <div className="!min-h-screen !min-w-full bg-brand-100 dark:bg-dark z-20">
            {showNavbar && <AuthNavbar showHamburgerIcon={showHamburgerIcon} hideMenu={hideMenu} showPlans={false} showAuthAccount={showAuthAccount} />}
