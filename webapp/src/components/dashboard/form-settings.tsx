...
export default function FormSettingsTab({view = 'DEFAULT'}: IFormSettingsTabProps) {
    ...
    const onVisibilityChanged = ({isPrivate = false, isHidden = false, f}: {
        isPrivate?: boolean;
        isHidden?: boolean;
        f?: StandardFormDto
    }) => {
                    ...
                    <FormSettingsCard className={'mb-4'}>
                        ...
                        <RadioGroup className="flex flex-col gap-6" value={currentVisibility}>
                            ...
                            <div className="flex flex-col">
                                ...
                                />
                                <span
                                    className="ml-8 body4 !text-black-700">{!(form?.groups.length === 0) ? 'Only members in this group can see this form.' : 'No Groups Selected.'}</span>
            ...
            case 'LINKS':
                return (
                    <FormSettingsCard className={'!space-y-0 !mt-0'}>
                        <p className="w-full body4 !text-black-700 lg:w-[564px]">Create personalized form links that
                            align with their branding or specific requirements. A link slug is a brief, descriptive part
                            of a URL that identifies web page content.</p>
                        <div className={'flex flex-row gap-2 items-start py-1 '}>
                            <p className="body4 !text-black-700 mt-1 mb-11 truncate">
                                {isCustomDomain ? customDomain : clientHost}/<span
                        ...
                        <div className={'flex flex-col gap-16'}>
                            {isCustomDomain &&
                                <FormLinkUpdateView isCustomDomain={isCustomDomain} link={customDomainUrl}
                                                    isProUser={!isAdmin || workspace?.isPro}/>}
                            <FormLinkUpdateView isCustomDomain={false}
                                                link={clientHostUrl}
...
const FormGroups = ({groups}: { groups: ResponderGroupDto[] }) => {
    ...
    return (
        <div className={'flex flex-col gap-0.5 mt-2'}>
            ...
            <div className={'mt-2'}>
                <AppButton onClick={() => fullScreenModal.openModal('SELECT_GROUP_FULL_MODAL_VIEW')} icon={<GroupIcon/>}
