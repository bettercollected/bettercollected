...
export default function EditWorkspaceDialog() {
    ...
    useEffect(() => {
        async function fetchWorkspaceId() {
            const userWorkspaceResponse = await fetch(`${environments.API_ENDPOINT_HOST}/workspaces/mine`, {
                credentials: 'include'
            });
            const userWorkspace = (await userWorkspaceResponse?.json().catch((e: any) => e))?.payload?.content ?? null;
            const { id } = userWorkspace[0];
            return id;
            const workspaceDetails = await fetch(`${environments.API_ENDPOINT_HOST}/workspaces/${id}`, {
                credentials: 'include',
                headers: {
        ...
        fetchWorkspaceId().then((d) => setWorkspaceId(d));
    }, []);
            ...
            <div className="w-full bg-white rounded-lg shadow-md md:mt-0 sm:max-w-lg xl:p-0">
                <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <form className="space-y-4 md:space-y-6" action="#">
                        ...
                        <Button
                            onClick={() => closeModal()}
                            className="!w-full border-[1px] text-gray-700 !bg-white border-gray-300  hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800"
                        >
                            Cancel
                        </Button>
                    </form>
