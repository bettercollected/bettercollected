...
export default function FormPageLayout(props: any) {
    ...
    const formLinkView = (url: string) => (
        <div key={url}
             className="text-black-900 space-x-4 underline w-full body4 items-center rounded p-4 flex flex-col bg-brand-100">
            <Tooltip title={url}>
    ...
    return (
        <SidebarLayout DrawerComponent={FormDrawer}>
            <div className="py-5 w-full relative">
                ...
                <div
                    className="absolute xl:left-[-40px] px-5 xl:px-10 pb-10 mt-16 top-0 w-full py-6 xl:max-w-289-calc-289">{props.children}</div>
                <MuiDrawer
                        ...
                        <Box sx={{overflow: 'auto', height: '100%'}}>
                            <div className=" px-5 h-full py-6 relative w-full">
                                <Close onClick={handleDrawerToggle}
                                       className="absolute blocks lg:hidden right-5 top-5 cursor-pointer"/>
