...
export default function AddFormOnGroup({ responderGroups, form }: IAddFormOnGroupProps) {
    ...
    return (
        <div className="p-10 relative bg-white md:w-[658px] rounded-[8px]">
            ...
            <h4 className="h4">Add {form.title} to Group </h4>
            <p className="mt-2 mb-8 body4  !text-black-700">Select groups to make form visible only to them.</p>
            <Autocomplete
                disablePortal
                id="form_list"
                className="mb-6 mt-5"
                ...
                renderOption={(props, option: ResponderGroupDto) => {
                    return (
                        <Tooltip title={isFormAlreadyInGroup(form.groups, option.id) ? `${form.title} is already on ${option.name}` : ''} key={option.id}>
                            <div>
