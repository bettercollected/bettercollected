        ...
        <div className="flex flex-col gap-4">
                ...
                <div className="flex  lg:flex-col lg:items-end gap-4">
                    <div className="flex flex-col gap-2 lg:items-end">
                        <div className="flex items-center gap-4">
                            <span className="h5-new text-black-700">Collaborators ({data?.length || 1 - 1})</span>
                            <div className="lg:hidden">
                        ...
                        <div>
                            {data?.map((user) => (
                                <div key={user.email}>
                                    <AuthAccountProfileImage image={user.profileImage}
            ...
            <div className=" hidden lg:flex justify-between">
                <div className="flex gap-2">
                    <AppButton onClick={onClickEditButton} icon={<EditIcon/>}
                               variant={ButtonVariant.Ghost}>Edit</AppButton>
                    <AppButton onClick={onClickOpenLinkButton} icon={<EyeIcon width={20} height={20}/>}
                               variant={ButtonVariant.Ghost}>Open
                        Link</AppButton>
                    <AppButton onClick={onClickShareWorkspaceButton} icon={<ShareIcon height={20} width={20}/>}
                               variant={ButtonVariant.Ghost}>Share
                        Workspace</AppButton>
                </div>
                <InviteCollaboratorButton onClick={onClickInviteCollaboratorButton}/>
            </div>
