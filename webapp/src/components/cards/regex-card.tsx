        ...
        <div className=" p-6 bg-white  rounded">
            <div className="flex flex-col md:flex-row justify-between items-start  md:items-center">
                ...
                {regex?.length === 0 && isAdmin && (
                    <AppButton variant={ButtonVariant.Ghost} icon={<Plus className="h-4 w-4"/>}
                               onClick={() => openModal('ADD_REGEX', {handleRegex: handleRegex})}>
            ...
            {!!regex && !isEmptyString(regex) && (
                <>
                    ...
                    <div
                        className="px-2 py-3 border-2 border-black-400  rounded flex items-center justify-between md:w-[400px] body4">
                        <Typography noWrap className="!text-black-800">
                            {regex}
                        </Typography>
                        {isAdmin && (
                            <span onClick={() => handleRegex(regex, handleRegexType.REMOVE)}
