                        ...
                        <div className="flex justify-between flex-col xs:flex-row">
                            ...
                            {isAdmin && (
                                <AppButton variant={ButtonVariant.Ghost}
                                           ...
                                           onClick={() => router.push(`/${workspace?.workspaceName}/dashboard/responders-groups/create-group`)}>

                                    <Typography
                                        className="!text-brand-500  body6"> {t(groupConstant.createGroup)}</Typography>
                                </AppButton>
                            )}
                        </div>
                ...
                {data &&
                    data?.map((group: ResponderGroupDto) => {
                        return <GroupCard key={group.id} responderGroup={group}
                                          handleDelete={() => handleDeletegroup(group)}/>;
                    })}
            </div>
        </div>
    );
