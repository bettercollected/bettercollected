...
const examples = [
    "Contact form to gathers user’s information. Include fields for the user's name, email address, and phone number.",
    "A form to collect suggestions for improving the design and functionality of a app."
]
    ...
    return (
        <form onSubmit={handleSubmit}>
            ...
            <div className="px-10 py-6 flex flex-col gap-4">
                ...
                <div className="bg-black-200 prompt-input-wrapper p-[1px] rounded-lg">
                    <div className="bg-white rounded-lg">
                        <textarea
                            className=" w-full text-sm rounded-lg border-none  placeholder-black-500"
                            style={{
                                resize: "none"
                            }}
                            rows={4}
                ...
                <div className="flex justify-between gap-4">
                    <div>
                        <Collapsible open={isOpen} onOpenChange={setIsOpen}>
                            <CollapsibleTrigger asChild>
                                <div
                                    className="flex gap-2 items-center text-black-700 text-sm mb-2 font-medium cursor-pointer">
                                    Examples <ChevronDown/>
                                </div>
                            </CollapsibleTrigger>
                            <CollapsibleContent
                                className={cn(" flex flex-col gap-2 transition-all duration-300", isOpen ? "h-auto" : "h-0")}>
                                {
                                    examples.map((example: string) => {
                                        return <GradiantBorderDiv key={example}
                                                                  onClick={() => {
                                                                      setPrompt(example)
                                                                  }}>
                                            <div
                                                className="bg-black-200 p-4 text-sm rounded-lg text-black-600 cursor-pointer"
                                            >
                                                {example}
                                            </div>
                                        </GradiantBorderDiv>
                    ...
                    <GenerateButton size={ButtonSize.Medium}
