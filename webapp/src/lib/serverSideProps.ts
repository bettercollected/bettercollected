...
export async function getServerSidePropsForDashboardFormPage(_context: any) {
    ...
    const config = getServerSideAuthHeaderConfig(_context);
    try {
        const formResponse = await fetch(`${environments.INTERNAL_DOCKER_API_ENDPOINT_HOST}/workspaces/${globalProps.workspace?.id}/forms/${form_id}?published=true&draft=true`, config);
