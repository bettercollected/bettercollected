...
export default function OtpEmailInput(props: OtpEmailInputPropType) {
    ...
    return (
        <form className={` w-full ${isModal ? ' mt-16' : ''}`} onSubmit={handleEmailInputForCreator}>
                    ...
                    <div className="mt-10 flex w-full gap-[20px]">
                        <div className="flex w-full flex-col items-center justify-center gap-4  sm:flex-row lg:gap-6">
                            {providers.map((provider: string) => (
                                <ConnectWithProviderButton key={provider} type={'dark'} url={`${environments.API_ENDPOINT_HOST}/auth/${provider}/basic`} text={`Sign in with ${capitalize(provider)}`} creator={true} fromProPlan={fromProPlan!} />
                            ))}
                        </div>
                    </div>
            ...
            <AppButton type={'submit'} variant={ButtonVariant.Primary} isLoading={creatorResponse.isLoading} className={`mt-6 w-full ${isModal ? 'mb-10' : ''}`} size={ButtonSize.Medium}>
