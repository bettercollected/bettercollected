...
export default function DeleteAccountModal() {
    ...
    return <BottomSheetModalWrapper>
        <div>
        ...
        <div className="mt-[72px] max-w-[540px]">
            <div className="h4-new text-black-800 mb-2">
                {t("DELETE_ACCOUNT.WHY_DELETE")}<span className="text-red-500 ml-2">*</span>
            </div>
            ...
            <div className="mt-10">
                <div className="h4-new text-black-800 mb-2">
                    {t("DELETE_ACCOUNT.TYPE_CONFIRM")}<span className="text-red-500 ml-2">*</span>
                </div>
                <AppTextField
                    onCut={handleCopyPaste}
                    onPaste={handleCopyPaste}
                    onCopy={handleCopyPaste}
                    value={confirm} onChange={(event) => {
                    setConfirm(event.target.value.toUpperCase())
                }} placeholder={"CONFIRM"}/>
            </div>
            <FormGroup className="mt-10 pl-2">
                <FormControlLabel checked={checked} onChange={(event, checked) => {
                    setChecked(checked)
                }} className="flex items-start" control={<CheckBox className="mr-2"/>}
                                  label={
                                      <div>
                                          {t("DELETE_ACCOUNT.I_UNDERSTAND_CONSEQUENCES")}<span
                                          className="text-red-500 ml-2">*</span>
                                      </div>
                                  }/>
            </FormGroup>
            <div className="mt-[72px]">
                {error && <div className="mb-4 text-sm text-red-500">
                    * Please fill in all required fields or check CONFIRM field.
                </div>}
