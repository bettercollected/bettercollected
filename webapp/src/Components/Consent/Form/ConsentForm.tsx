...
export default function ConsentForm({className, onFormSubmit, form, isDisabled = false}: ConsentBuilderProps) {
    ...
    const getFilteredConsents = (category: ConsentCategoryType) => {
        return form?.consent?.filter((consent) => consent?.category === category).map((consent, idx) => <ConsentField
            key={consent.consentId} className={`${idx === 0 && 'border-y'}`} consent={consent} disabled={isDisabled}/>);
    };
            ...
            <div className="space-y-5">
                ...
                <div className="border-y border-new-black-300 py-5 space-y-2">
                    ...
                    <ul className="space-y-3 list-disc !px-5">
                        ...
                        <li className="p2">We will not sell or share your data with any other third parties without your
                            consent.
                        </li>
                        <li className="p2">We will take all reasonable steps to protect your data from unauthorized
        ...
        if (responderRight) {
            return (
                <div className="space-y-5">
                    ...
                    <div className="h5-newborder-y border-new-black-300 py-5">You can request for deletion of your data
                        at any time
                    </div>
                </div>
            );
        ...
        <form className={cn(className)} onSubmit={onSubmit}>
            <div className="space-y-20 xs:space-y-[70px]">
                <div className="space-y-4">
                    ...
                    <div
                        className="p2 !text-new-black-800 ">{`We want to make sure you're fully informed about how your data will be used before you proceed with our form.`}</div>
                </div>
                {renderFormPurposes()}
                {renderDataRetention()}
