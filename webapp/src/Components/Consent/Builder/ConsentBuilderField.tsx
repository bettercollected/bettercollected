...
export default function ConsentBuilderField({
                                                consent,
                                                className,
                                                hint,
                                                disabled = false,
    ...
    if (consent.category === ConsentCategoryType.DataRetention) {
        return (
            <div className="space-y-3">
                <div id={`item-${consent.consentId}`}
                     className={cn('flex items-center justify-between space-y-2 p-5 rounded-lg  bg-new-black-200 group', className, disabled && 'hover:cursor-default')}>
                    <div className="h6-new">{consent.title}</div>
                    <DropdownCloseIcon className={cn('!m-0 hover:cursor-pointer', disabled && 'hidden')}
    ...
    return (
        ...
        >
            <div className="flex items-center justify-between">
                ...
                <DropdownCloseIcon className={cn('!m-0 hidden group-hover:block', disabled && 'group-hover:hidden')}
                                   onClick={handleRemoveConsent}/>
            </div>
