...
import {
    addDuplicateField, resetBuilderMenuState,
    setAddNewField,
    setDeleteField,
    setIdentifierField,
    setUpdateField
...
export default function FieldOptions({provided, id, position}: IFieldOptionsProps) {
    ...
    return (
        <MenuDropdown
            ...
            menuContent={
                <div
                    className={`flex items-center w-9 justify-center cursor-pointer rounded-sm text-neutral-400 `} {...provided.dragHandleProps}
                    tabIndex={-1}>
                    <DragHandleIcon className={isMobile && builderState.activeFieldId === id ? "text-black-800" : ""}
                                    tabIndex={-1} width={24} height={24}/>
                </div>
            }
        >
            <div
            ...
            >
                ...
                <MenuItem className="md:hidden" onClick={() => {
                    batch(() => {
                        setOpen(false);
                ...
                <MenuItem sx={{paddingX: '20px', paddingY: '10px', height: '30px'}}
                          className="flex items-center body4 !text-black-700 hover:bg-brand-100"
                    ...
                    <MenuItem sx={{paddingX: '20px', paddingY: '10px'}}
                        ...
                        <FormControlLabel
                            ...
                            control={<MuiSwitch sx={{m: 1}} className="text-black-900 m-0" size="small"
                                                onChange={handleSetEmailIdentifier}
                                                checked={responseOwnerField === field?.id}/>}
                        />
                    </MenuItem>
                )}
                ...
                {!hasLabelField() && (<>
                        <Divider/>
                        <MenuItem sx={{paddingX: '20px', paddingY: '10px', height: '30px'}}
                                  className="flex items-center body4 !text-black-700 xl:hidden hover:bg-brand-100"
                                  onClick={addFieldLabel}>
                            <ListItemIcon className=" rounded text-black-900">
                            <span
                                className="bg-black-100 w-5 h-5 text-center justify-center font-bold text-[14px] flex items-center">L</span>
                            </ListItemIcon>
                            <span className="leading-none flex items-center justify-between w-full">
